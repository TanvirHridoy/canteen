<?php
					...
					if(isset($_REQUEST['ds']))
					{
						...
						$delQuery = $conn1->prepare("DELETE FROM `event` WHERE `id`=:GOT_ID");
...
$usersList = $conn1->query("SELECT * FROM `event` order by `id` desc");
...
foreach($usersList AS $eachRow)
{
	...
	<tr>
		...
		<a href="list-event.php?ds='.$eachRow['id'].'" > <input name="ds" type="submit" value="Delete"/></a>
		</td>
